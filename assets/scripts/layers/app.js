...
import { rndId, clamp, dist, lerp, lpt, nearly, rotatePoint, itemPoints, setItemPoints, itemCenter, pointLineDist, pointQuadNear, pointInPolygon, getCssVar } from '../core/utils.js';
...
// ========= Drawing / Render =========
        ...
        function stepPlay(now) {
            ...
            const durSec = Math.max(0.001, a.duration || 5);
            ...
            state.tl.sec = Math.min(Math.floor(t), Math.round(durSec));
            let k1 = ks[0],
                k2 = ks[ks.length - 1];
            ...
            UI.playhead.style.left = secToX(endReached ? durSec : t) + 'px';
            placeCursor();
            if (endReached) {
                state.tl.playing = false;
                return;
